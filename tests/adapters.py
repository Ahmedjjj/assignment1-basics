...
def run_multihead_self_attention(
    ...
    """
    m_s_attention = MultiHeadSelfAttention(d_model=d_model, num_heads=num_heads)

    m_s_attention.load_state_dict(
        {
            "linear.weights": torch.concat((q_proj_weight, k_proj_weight, v_proj_weight)),
            "linear_o.weights": o_proj_weight,
        }
    )

    return m_s_attention.forward(in_features)
...
def run_multihead_self_attention_with_rope(
    ...
    m_s_attention = MultiHeadSelfAttention(d_model=d_model, num_heads=num_heads, rope=rope)
    ...
    return m_s_attention.forward(in_features, token_positions=token_positions)
...
def run_rope(
    ...
    """
    return RoPE(theta=theta, d_k=d_k, max_seq_len=max_seq_len).forward(
        x=in_query_or_key, token_positions=token_positions
    )
...
def run_transformer_block(
    ...
    """
    rope = RoPE(theta=theta, d_k=d_model // num_heads, max_seq_len=max_seq_len)
    transformer = TransformerBlock(d_model=d_model, num_heads=num_heads, d_ff=d_ff, rope=rope)
    ...
    transformer.load_state_dict(
        {
            "input_rms_norm.weights": weights["ln1.weight"],
            "attention.linear.weights": torch.concat((q_proj_weight, k_proj_weight, v_proj_weight)),
            "attention.linear_o.weights": o_proj_weight,
            "ffn_rms_norm.weights": weights["ln2.weight"],
...
def run_train_bpe(
    ...
    **kwargs,
    ...
    tokenizer = train_bpe(texts=corpus, special_tokens=special_tokens, vocab_size=vocab_size)
